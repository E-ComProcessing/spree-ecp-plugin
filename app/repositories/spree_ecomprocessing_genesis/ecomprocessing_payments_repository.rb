module SpreeEcomprocessingGenesis
  # Ecomprocessing Payments Repository
  class EcomprocessingPaymentsRepository

    class << self

      # Find by transaction_id
      def find_by_transaction_id(transaction_id)
        Db::EcomprocessingPayment.find_by(transaction_id: transaction_id)
      end

      # Find by unique_id
      def find_by_unique_id(unique_id)
        Db::EcomprocessingPayment.find_by(unique_id: unique_id)
      end

      # Find all payments that belong to an order
      def find_all_by_order_and_payment(order_id, payment_id)
        Db::EcomprocessingPayment.where(order_id: order_id, payment_id: payment_id).order('id desc')
      end

      # Store Genesis Payment to the DB
      def save_from_response_data(genesis_request, genesis_response, order, spree_payment)
        payment                   = Db::EcomprocessingPayment.new
      ...
      def update_from_response_data(ecomprocessing_payment, response_object, spree_payment)
        request = { configuration: {
          token: ecomprocessing_payment.terminal_token || response_object[:terminal_token]
        } }.with_indifferent_access
        response_object[:mode] = ecomprocessing_payment.mode unless response_object.key? :mode

        map_payment spree_payment, ecomprocessing_payment, request, response_object
      ...
      # Map the given data to the Ecomprocessing Payment
