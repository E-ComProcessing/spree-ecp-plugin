module Spree
  module CheckoutControllerDecorator
    ...
    def update
      result = process_ecp_direct_payment? ? process_ecp_direct_payment : default_payment_processing
    ...
    # Check for Ecomprocessing Direct Payment process
    def process_ecp_direct_payment?
      @order.state == 'payment' && ecomprocessing_direct_payment?
    end

    # Check if the current payment method is Ecomprocessing Direct
    def ecomprocessing_direct_payment?
      return false unless payment_method_id

      payment_method = Spree::PaymentMethod.find(payment_method_id)

      payment_method&.type == Spree::Gateway::EcomprocessingDirect.name
    end

    # Process Ecomprocessing Direct Payment
    def process_ecp_direct_payment
