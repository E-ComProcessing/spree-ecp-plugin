module SpreeEcomprocessingGenesis
  ...
  class GenesisProvider # rubocop:disable Metrics/ClassLength
    ...
    # Handle notification
    def notification(ecomprocessing_payment, params)
      ...
      handle_reconciliation_response ecomprocessing_payment, notification.reconciliation

      notification
    end

    # Execute 3DSv2 Method Continue
    def method_continue(ecomprocessing_payment)
      method_continue = TransactionHelper.init_method_continue_req @configuration

      genesis = GenesisRuby::Genesis.new(
        @configuration, Mappers::Genesis.for_method_continue(method_continue, ecomprocessing_payment).context
      )
    ...
    def process_reference_response(original_transaction, genesis_request, genesis_response)
      ...
      if TransactionHelper.success_result? genesis_response
        ...
        EcomprocessingPaymentsRepository.save_reference_from_transaction(
    ...
    def handle_response(request, response, is_payment: true)
      ...
      if TransactionHelper.can_save_genesis_response? response_object
        EcomprocessingPaymentsRepository.save_from_response_data request, response_object, @order, @payment
      end
    ...
    def handle_reconciliation_response(ecomprocessing_payment, reconciliation)
      response_object = fetch_reconciliation_object reconciliation, ecomprocessing_payment.unique_id

      EcomprocessingPaymentsRepository.update_from_response_data ecomprocessing_payment, response_object, @payment
