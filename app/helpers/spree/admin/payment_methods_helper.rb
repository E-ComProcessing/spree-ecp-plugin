    ...
    module PaymentMethodsHelper
      ...
      def add_select_preference(form, field, options, html_options)
        form.select(
          field,
          options_for_select(
            options[:values].map { |key| [I18n.t(key, scope: 'ecomprocessing.preferences'), key] }, options[:selected]
          ),
          {},
          html_options
        )
      ...
      def default_preference_support?(object, preference)
        unsupported_default_preferences.include?(preference.to_s) &&
          (
            object.instance_of?(Spree::Gateway::EcomprocessingDirect) ||
              object.instance_of?(Spree::Gateway::EcomprocessingCheckout)
          )
      end

      # Not Supported preferences from Ecomprocessing Gateway
