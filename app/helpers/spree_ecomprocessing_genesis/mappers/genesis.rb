...
module SpreeEcomprocessingGenesis
  module Mappers
    ...
    class Genesis # rubocop:disable Metrics/ClassLength
      ...
      def self.for_method_continue(genesis, ecomprocessing_payment)
        this = new genesis

        this.map_method_continue ecomprocessing_payment
      end
      ...
      def map_reference(amount, transaction, order)
        ...
        @context.usage          =
          "#{I18n.t("reference", scope: "ecomprocessing.payment")} - #{transaction.transaction_type.capitalize}"
      ...
      def map_config(options)
        ...
        @context.endpoint    = GenesisRuby::Api::Constants::Endpoints::ECOMPROCESSING

        self
      end

      # Map payment data to hash used for 3DS method continue execution
      def map_method_continue(ecomprocessing_payment)
        @context.transaction_unique_id = ecomprocessing_payment.unique_id
        @context.amount                = ecomprocessing_payment.amount
        @context.transaction_timestamp = ecomprocessing_payment.zulu_response_timestamp
      ...
      def common_attributes(provider, support_ip: true) # rubocop:disable Metrics/AbcSize
        ...
        @context.usage          = I18n.t 'usage', scope: 'ecomprocessing.payment'
      ...
      def asyn_attributes(options, is_wpf: false)
        @context.notification_url   = "#{options[:hostname]}#{api_v2_storefront_ecomprocessing_notification_path}"
