  ...
  module Mappers
    ...
    class Order
      ...
      def self.for(raw_data)
        mapped_data = new SpreeEcomprocessingGenesis::Data::Provider.new
      ...
      def map_user(data, node_key, attr_value)
        data.public_send node_key, map_order_data(SpreeEcomprocessingGenesis::Data::User.new, attr_value)
      end

      # Map Addresses
      def map_address(data, node_key, attr_value)
        data.public_send node_key, map_order_data(SpreeEcomprocessingGenesis::Data::Address.new, attr_value)
      end
