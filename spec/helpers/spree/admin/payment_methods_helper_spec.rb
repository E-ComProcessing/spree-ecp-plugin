...
RSpec.describe Spree::Admin::PaymentMethodsHelper do
  ...
  let(:helper) { SpreeEcomprocessingGenesisSpec::Stubs::PaymentMethodHelperStub.new }
  let(:payment_method) { create(:ecomprocessing_direct_gateway) }
  let(:options) { { type: :string, values: nil, selected: 'selected' } }
  let(:form) do
    ActionView::Helpers::FormBuilder.new(
      'gateway_ecomprocessing_direct',
      payment_method,
      view,
      {
        url:  "/admin/payment_methods/#{payment_method.id}",
        html: {
          class:              'edit_gateway_ecomprocessing_direct',
          id:                 "edit_gateway_ecomprocessing_direct_#{payment_method.id}",
          method:             :patch,
