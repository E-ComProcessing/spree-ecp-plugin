RSpec.describe SpreeEcomprocessingGenesis::ThreedsHelper do
  ...
  it 'when fetch_shipping_address_first_used with shipping_address' do
    expect(described_class.fetch_shipping_address_first_used(shipping_address))
      .to eq Date.current.last_year.strftime(SpreeEcomprocessingGenesis::ThreedsHelper::DATE_ISO_FORMAT)
  end

  it 'when fetch_class_indicator with lest than 30 days' do
    expect(described_class.fetch_class_indicator(
             'UpdateIndicators',
             Date.current.days_ago(2).strftime(SpreeEcomprocessingGenesis::ThreedsHelper::DATE_ISO_FORMAT)
  ...
  it 'when fetch_class_indicator with more than 30 days and less than 60' do
    expect(described_class.fetch_class_indicator(
             'UpdateIndicators',
             Date.current.days_ago(33).strftime(SpreeEcomprocessingGenesis::ThreedsHelper::DATE_ISO_FORMAT)
           )).to eq '30_to_60_days'
  end
  ...
  it 'when fetch_class_indicator with more than than 60' do
    expect(described_class.fetch_class_indicator(
             'UpdateIndicators',
             Date.current.days_ago(66).strftime(SpreeEcomprocessingGenesis::ThreedsHelper::DATE_ISO_FORMAT)
