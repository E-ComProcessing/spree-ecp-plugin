...
RSpec.describe SpreeEcomprocessingGenesis::Mappers::Genesis do
  ...
  let(:payment) do
    described_class.for_payment(gate_request, mapped_order_with_address, ecomprocessing_source, options).context
  end
  ...
  describe 'when config' do
    ...
    it 'with proper endpoint' do
      expect(config.endpoint).to eq 'e-comprocessing.net'
    end
    ...
    describe 'when production' do
      let(:config) do
        described_class.for_config(
          create(:ecomprocessing_direct_gateway, preferred_test_mode: 'false').preferences
        ).context
      end
  ...
  describe 'when payment without order' do
    ...
    describe 'with missing data' do
      ...
      let(:payment) { described_class.for_payment(gate_request, mapped_order, ecomprocessing_source, options).context }
    ...
    describe 'with complete data' do
      ...
      it 'with notification_url' do
        expect(payment.notification_url).to eq 'http://127.0.0.1:4000/api/v2/storefront/ecomprocessing_notification'
      end
      ...
      it 'with threeds_v2_method_callback_url' do
        expect(payment.threeds_v2_method_callback_url)
          .to eq 'http://127.0.0.1:4000/api/v2/storefront/ecomprocessing_threeds/status'
      end
  ...
  describe 'when reference' do
    let(:reference) do
      ...
      described_class.for_reference(
        ...
        99,
        SpreeEcomprocessingGenesis::EcomprocessingPaymentsRepository.find_by_transaction_id('123456'),
        SpreeEcomprocessingGenesis::Mappers::Order.for(spree_payment.order.attributes.symbolize_keys)
      ).context
    end
  ...
  describe 'when wpf' do
    let(:source) do
      create :ecomprocessing_checkout_source,
             public_metadata: {
    ...
    it 'with default transaction types' do # rubocop:disable RSpec/ExampleLength
      options = described_class.for_urls!(
        Spree::Gateway::EcomprocessingCheckout.new.preferences, spree_payment.order.number
      )
