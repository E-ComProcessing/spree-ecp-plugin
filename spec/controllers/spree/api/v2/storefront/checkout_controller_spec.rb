RSpec.describe Spree::Api::V2::Storefront::CheckoutController, :vcr, type: :controller do
  ...
  describe 'when create_payment' do
    describe 'when ecomprocessing direct payment' do
      let(:payment) do
      ...
      describe 'when successful payment' do # rubocop:disable RSpec/NestedGroups
        it 'with approved payment state' do
          ...
          expect(JSON.parse(response.body)['data']['ecomprocessing_payment'])
        ...
        it 'with pending payment state with 3dsv2 method continue' do
          ...
          expect(JSON.parse(response.body)['data']['ecomprocessing_payment'])
            .to eq({ 'state' => 'pending_async', 'redirect_url' => 'http://127.0.0.1:4000/ecomprocessing_threeds/' \
        ...
        it 'with pending payment with redirect url' do
          ...
          expect(JSON.parse(response.body)['data']['ecomprocessing_payment'])
            .to eq({ 'state' => 'pending_async', 'redirect_url' => 'https://staging.gate.e-comprocessing.net/threeds/' \
                   'authentication/162d90bf750a62392b12a88010426ccd' })
        end
      end
    end

    describe 'when ecomprocessing checkout payment' do
  ...
  describe 'when checkout_method' do
    let(:payment) do
      create :spree_payment,
             payment_method: create(:ecomprocessing_checkout_gateway),
    ...
    it 'when complete with success response' do
      ...
      redirect_url = 'https://staging.wpf.e-comprocessing.net/en/v2/payment/9aed8de9319c4c3806a9d1614797f853'
  ...
  describe 'when non ecomprocessing payment method' do
    let(:order) { OrderWalkthrough.up_to :payment }
