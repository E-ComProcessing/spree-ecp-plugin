RSpec.describe Spree::Api::V2::Storefront::EcomprocessingThreedsController, :vcr, type: :controller do
  ...
  describe 'when valid params' do
    ...
    describe 'when callback_handler' do
      let(:params) do
        {
          ...
          signature:             Digest::SHA512.hexdigest(
            "#{ecomprocessing_payment.unique_id}completed#{payment.payment_method.preferences[:password]}"
          )
        }
      end
      ...
      it 'with callback_status update' do # rubocop:disable RSpec/MultipleExpectations
        expect(ecomprocessing_payment.reload.callback_status).to be_nil

        post :callback_handler, params: params

        expect(ecomprocessing_payment.reload.callback_status).to eq 'completed'
      end
    ...
    describe 'when callback_status' do
      let(:update_callback_status) do
        ecomprocessing_payment.callback_status = 'updated_status'
        ecomprocessing_payment.save!
      end

      it 'without updated callback_status' do
        get :callback_status, params: { unique_id: ecomprocessing_payment.unique_id }
    ...
    describe 'when method_continue' do
      let(:ecomprocessing_pending_payment) do
        create :ecomprocessing_direct_payment,
      ...
      it 'with proper method_continue request with pending payment' do # rubocop:disable RSpec/ExampleLength
        post :method_continue, params: {
          unique_id: ecomprocessing_pending_payment.unique_id,
          signature: GenesisRuby::Utils::Threeds::V2.generate_signature(
            unique_id:         ecomprocessing_pending_payment.unique_id,
            amount:            ecomprocessing_pending_payment.amount,
            timestamp:         ecomprocessing_pending_payment.zulu_response_timestamp,
            merchant_password: payment.payment_method.preferences[:password]
          )
        }

        expect(response.body).to eq({ status: 'OK', redirect_url: 'https://staging.gate.e-comprocessing.net/threeds/' \
