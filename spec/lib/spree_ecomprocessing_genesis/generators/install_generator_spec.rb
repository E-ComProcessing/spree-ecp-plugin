require 'fileutils'
require 'generators/spree_ecomprocessing_genesis/install/install_generator'

# rubocop:disable RSpec/MultipleMemoizedHelpers
RSpec.describe SpreeEcomprocessingGenesis::Generators::InstallGenerator do
  ...
  let(:reset_migrations!) do
    FileUtils.rm_rf Dir.glob(File.join('spec/dummy', 'db/**/*spree_ecomprocessing_genesis.rb'))
  end
  let(:drop_engine_tables!) do
    ...
    ActiveRecord::Base.connection.execute 'DROP TABLE ecomprocessing_payments;'
  end

  before do
    stub_const(
      'SpreeEcomprocessingGenesis::Generators::InstallGenerator::RAKE_PATH',
  ...
  describe 'when frontend' do
    before do
      stub_const('SpreeEcomprocessingGenesis::Generators::InstallGenerator::JS_FRONTEND_PATH', js_path)
      stub_const('SpreeEcomprocessingGenesis::Generators::InstallGenerator::CSS_FRONTEND_PATH', css_path)
    end
  ...
  it 'when add_migrations' do
    ...
    expect(Dir.glob(File.join('spec/dummy', 'db/**/*spree_ecomprocessing_genesis.rb')).count).to eq 3
  end
  ...
  it 'when run_migrations' do # rubocop:disable RSpec/ExampleLength
    ...
    expect(ActiveRecord::Base.connection.tables).to include 'ecomprocessing_payments'
  end
