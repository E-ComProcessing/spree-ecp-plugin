...
RSpec.describe Spree::Gateway::EcomprocessingCheckout, :vcr do
  let(:gateway) { create :ecomprocessing_checkout_gateway, preferred_language: 'bg' }
  ...
  describe 'when behaviour' do
    ...
    it 'with proper source class' do
      expect(gateway.payment_source_class.name).to eq Spree::EcomprocessingCheckoutSource.name
    end
    ...
    it 'with method type' do
      expect(gateway.method_type).to eq 'ecomprocessing_checkout'
    end
