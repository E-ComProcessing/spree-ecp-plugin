...
RSpec.describe Spree::Gateway::EcomprocessingDirect, :vcr do
  let(:gateway) { create :ecomprocessing_direct_gateway }
  ...
  describe 'when behaviour' do
    ...
    it 'when sale type with enabled auto capture' do
      gateway = create :ecomprocessing_direct_gateway, transaction_types: 'sale3d'

      expect(gateway.auto_capture?).to be true
    end

    it 'with provider' do
      expect(gateway.provider).to be_a SpreeEcomprocessingGenesis::GenesisProvider
    end
    ...
    it 'with method type' do
      expect(gateway.method_type).to eq 'ecomprocessing_direct'
    end
  ...
  describe 'when purchase with order' do
    ...
    describe 'when sale payment' do
      let(:gateway) { create :ecomprocessing_direct_gateway, transaction_types: 'sale3d' }
    ...
    describe 'when refund' do
      ...
      it 'with first level final transaction' do # rubocop:disable RSpec/ExampleLength
        transaction = create :ecomprocessing_payment,
                             ...
                             reference_id:     nil,
                             payment_method:   ecomprocessing_payment.payment_method.name,
                             transaction_type: 'sale3d',
                             payment_id:       ecomprocessing_payment.number,
                             order_id:         ecomprocessing_payment.order.number,
                             amount:           amount_in_cents,
                             currency:         ecomprocessing_payment.order.currency
        refund      = {
          originator: create(:refund, amount: ecomprocessing_payment.amount, payment_id: ecomprocessing_payment.id)
        }
