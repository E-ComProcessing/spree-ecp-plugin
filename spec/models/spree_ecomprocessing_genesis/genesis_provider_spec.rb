...
RSpec.describe SpreeEcomprocessingGenesis::GenesisProvider, :vcr do
  ...
  describe 'when error handling' do
    let(:genesis_provider) do
      described_class.new SpreeEcomprocessingGenesis::PaymentMethodHelper::DIRECT_PAYMENT, options
    end

    describe 'when GenesisRuby Error' do
      before do
        allow(SpreeEcomprocessingGenesis::TransactionHelper)
    ...
    describe 'when Standard Error' do
      before do
        allow(SpreeEcomprocessingGenesis::TransactionHelper).to receive(:init_genesis_req).and_raise StandardError
      end
      ...
      it 'with reference' do # rubocop:disable RSpec/ExampleLength
        transaction = create :ecomprocessing_payment,
  ...
  describe 'when initialization' do
    ...
    it 'with checkout method type' do
      genesis_provider = described_class.new(
        SpreeEcomprocessingGenesis::PaymentMethodHelper::CHECKOUT_PAYMENT,
        spree_payment.payment_method.preferences
      )

      expect(genesis_provider.instance_variable_get(:@method_type)).to eq 'ecomprocessing_checkout'
    end
  ...
  describe 'when initialized' do
    ...
    let(:order_data) do
      SpreeEcomprocessingGenesis::Mappers::Order.prepare_data(
        spree_payment.order,
  ...
  describe 'ecomprocessing direct processing' do
    let(:order_data) do
      SpreeEcomprocessingGenesis::Mappers::Order.for(
        SpreeEcomprocessingGenesis::Mappers::Order.prepare_data(
          spree_payment.order,
          spree_payment.order.user,
          build(
    ...
    let(:genesis_provider) do
      provider = described_class.new SpreeEcomprocessingGenesis::PaymentMethodHelper::DIRECT_PAYMENT, options
      provider.load_data order_data
      provider.load_source(
        build(
          :ecomprocessing_credit_card_source,
    ...
    describe 'when purchase' do
      ...
      it 'with stored ecomprocessing payment' do
        response = genesis_provider.purchase
        payment  = SpreeEcomprocessingGenesis::EcomprocessingPaymentsRepository.find_by_transaction_id(
    ...
    describe 'when capture' do
      ...
      it 'with stored reference payment' do
        ...
        payment = SpreeEcomprocessingGenesis::EcomprocessingPaymentsRepository.find_by_unique_id transaction.unique_id
    ...
    describe 'when method continue' do
      let(:payment) do
        create :ecomprocessing_payment,
               ...
               response:         {
                 ...
                 threeds_method_continue_url: 'https://staging.gate.e-comprocessing.net/threeds/threeds_method/' \
      ...
      it 'with redirect_url after 3dsv2 continue request' do
        ...
        expect(response.response_object[:redirect_url])
          .to eq 'https://staging.gate.e-comprocessing.net/threeds/authentication/162d90bf750a62392b12a88010426ccd'
      end
    end
  ...
  describe 'ecomprocessing checkout web payment form' do
    let(:source) { create(:ecomprocessing_checkout_source) }
    let(:spree_payment) do
      create :spree_payment,
             order: create(:order_with_line_items, currency: 'EUR'),
             payment_method: source.payment_method,
    ...
    let(:genesis_provider) do
      provider = described_class.new SpreeEcomprocessingGenesis::PaymentMethodHelper::CHECKOUT_PAYMENT, options
      provider.load_data order_data
      provider.load_source(
        build(
          :ecomprocessing_checkout_source,
          payment_method: spree_payment.payment_method,
          user: spree_payment.order.user
        )
      )
