...
if [ "${USERS_TABLE}" -eq 0 ]; then
  ...
  echo "Install E-Comprocessing Gateway Module for Spree Commerce"
  bin/rails g spree_ecomprocessing_genesis:install --auto-run-migrations
...
CHECKOUT_METHOD=$(psql -U "${POSTGRES_USER}" -h "${PGHOST}" "${POSTGRES_DB}" -AXqtc "SELECT count(id) FROM spree_payment_methods where type='Spree::Gateway::EcomprocessingCheckout';")
if [ "${CHECKOUT_METHOD}" -eq 0 ]; then
  echo "Install ecomprocessing Checkout Payment Method"
  sh /bin/install_checkout_method.sh
fi

DIRECT_METHOD=$(psql -U "${POSTGRES_USER}" -h "${PGHOST}" "${POSTGRES_DB}" -AXqtc "SELECT count(id) FROM spree_payment_methods where type='Spree::Gateway::EcomprocessingDirect';")
if [ "${DIRECT_METHOD}" -eq 0 ]; then
  echo "Install ecomprocessing Direct Payment Method"
